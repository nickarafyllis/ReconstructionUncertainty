...
#{'colsample_bytree': 0.8, 'gamma': 0.2, 'learning_rate': 0.025, 'max_depth': 3, 'min_child_weight': 3, 'n_estimators': 300, 'subsample': 0.8}
}
...
n_jobs = -1 
model = xgb.XGBRegressor(n_jobs=n_jobs,**params)
model.fit(arr2_hi_E_train, arr3_hi_E_train, eval_set=eval_set)

# save the model to disk
filename = 'models/finalized_XGBoostmodel_forMuonEnergy_trueTrackLengthInMrd.sav'
pickle.dump(model, open(filename, 'wb'))


mse = mean_squared_error(arr3_hi_E_test, model.predict(arr2_hi_E_test)) 
print("MSE: %.4f" % mse)
...
# show the legend
plt.legend()
#plt.savefig("XGBoost_plots_trueMRD/deviation_train_test.png")